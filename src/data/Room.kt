...
import com.example.other.transformToUnderscore
...
class Room(
    ...
    private var currentThreeWordsToChoose: List<String>? = null
    ...
    private suspend fun sendCurrentRoundDrawInfo(player: Player) {
    ...
    suspend fun addPlayer(clientId: String, username: String, socket: WebSocketSession): Player {
        ...
        val temporaryMutablePlayersList = players.toMutableList()
        temporaryMutablePlayersList.add(indexToAdd, player)
        players = temporaryMutablePlayersList.toList()
        ...
        sendCurrentRoundDrawInfo(player)
    ...
    fun removePlayer(clientId: String) {
        ...
        playerRemoveJobs[clientId] = GlobalScope.launch {
            delay(PLAYER_REMOVE_TIME) // the code after delay is executed only if the coroutine is not cancelled
    ...
    private fun timeAndNotify(ms: Long) {
        ...
        timerJob = GlobalScope.launch {
            ...
            repeat((ms / UPDATE_TIME_FREQUENCY).toInt()) {
                ...
                phaseChange.ms -= UPDATE_TIME_FREQUENCY
    ...
    private suspend fun sendWordToPlayer(player: Player) {
        ...
        word?.let { currentWord ->
            drawingPlayer?.let { drawPlayer ->
                val gameState = GameState(
                    drawPlayer.username,
                    if (player.isDrawing || phase == Phase.SHOW_WORD) {
                        currentWord
                    } else {
                        currentWord.transformToUnderscore()
                    }
                )
    ...
    suspend fun checkWordAndNotifyPlayers(chatMessage: ChatMessage): Boolean {
        if (isGuessCorrect(chatMessage)) {
            ...
            val player = players.find{ it.username == chatMessage.from }
            ...
            val announcement = Announcement(
                "${chatMessage.from} guessed the word!",
                System.currentTimeMillis(),
                Announcement.TYPE_PLAYER_GUESSED_WORD
            )
            broadcast(gson.toJson(announcement))

            val isRoundOver = addWinningPlayer(chatMessage.from)
            if (isRoundOver) {
                val roundOverAnnouncement = Announcement(
                    "Everybody has guessed it! New round is starting...",
                    System.currentTimeMillis(),
    ...
    private fun newRound() {
        ...
        currentThreeWordsToChoose = getRandomWords()
        val newWords = NewWords(currentThreeWordsToChoose!!)
        nextDrawingPlayer()
    ...
    private fun gameRunning() {
        ...
        val wordToSend = word ?: currentThreeWordsToChoose?.random() ?: words.random()
        word = wordToSend
        val wordWithUnderscore = wordToSend.transformToUnderscore()
